...
class BridgeEngine:
    ...
    def start(self, chars: list[RuntimeCharacter]) -> None:
        ...
        for c in chars:
            ...
            t7 = threading.Thread(
                target=lambda *args: None, args=(combat_path,), daemon=True
            )
            t7.start()
...
class App:
    ...
    def _render_row(self, win: DetectedWindow):
        ...
        # Slot entry (editable)
        slot_entry = tk.Entry(
            row, bg="#0f172a", fg=ACCENT, insertbackground=FG, relief="flat", font=("Consolas", 10, "bold"), width=8
        )
        slot_txt = f"{win.slot}" if win.slot else ""
        slot_entry.insert(0, slot_txt)
        slot_entry.pack(side="left", padx=(0, 4))

        # Current window title (editable)
        title_entry = tk.Entry(
            row, bg="#0f172a", fg=FG, insertbackground=FG, relief="flat", font=("Segoe UI", 9), width=24
        )
